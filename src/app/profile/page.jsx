...
export default function DashboardPage() {
  ...
  const { clearTokens, userSession } = useAuthStore();
 
  const [isLoading, setIsLoading] = useState(!userSession);
  ...
  const handleLogout = async () => {
    try {
      const response = await fetch('/api/auth/set-token', { method: 'DELETE' });
      if (!response.ok) {
         console.error("API call to clear auth cookie failed:", response.statusText);
      }
    } catch (error) {
      console.error("Failed to send request to clear auth cookie:", error);
    }
    await clearTokens();
    router.push('/');
  };
