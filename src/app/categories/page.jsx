...
export default async function CategoryPage() {
  ...
  return (
    <section className="flex flex-col items-center justify-center pt-20 sm:pt-24 px-4 sm:px-12 mb-8">
      <div className="z-10 w-full min-h-[55vh] items-center justify-between text-sm font-mono">
        <h1 className="text-4xl font-bold mb-2">Categories</h1>
          ...
          {categoryData && categoryData.length > 0 ? (categoryData.map((category, index) => (
            <Link key={category.id || index} href={`/category?list=${category.id}`} className="card bg-secondary w-96 shadow-sm my-4">
